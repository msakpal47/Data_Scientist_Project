...
class FeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def _transform_raw(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if {"amnt_loans30", "sumamnt_ma_rech30"}.issubset(out.columns):
            out["loan_to_recharge_ratio_30"] = self._safe_div(out["amnt_loans30"], out["sumamnt_ma_rech30"] + 1)
        if {"amnt_loans90", "sumamnt_ma_rech90"}.issubset(out.columns):
            out["loan_to_recharge_ratio_90"] = self._safe_div(out["amnt_loans90"], out["sumamnt_ma_rech90"] + 1)

        if {"payback30", "cnt_loans30"}.issubset(out.columns):
            out["repayment_ratio_30"] = self._safe_div(out["payback30"], out["cnt_loans30"] + 1)
        if {"payback90", "cnt_loans90"}.issubset(out.columns):
            out["repayment_ratio_90"] = self._safe_div(out["payback90"], out["cnt_loans90"] + 1)

        if {"sumamnt_ma_rech30", "sumamnt_ma_rech90"}.issubset(out.columns):
            out["recharge_growth"] = self._safe_div(out["sumamnt_ma_rech30"], out["sumamnt_ma_rech90"] + 1)

        return out


def build_pipeline(random_state: int, model_type: str | None = None, scale_pos_weight: float | None = None) -> Pipeline:
    ...
    chosen = (model_type or "auto").lower()
    clf = None
    if chosen in ("xgb", "auto"):
        try:
            ...
            clf = None
    if clf is None and chosen in ("rf", "auto"):
