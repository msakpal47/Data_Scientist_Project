...
def infer_feature_columns(df: pd.DataFrame, target: str) -> List[str]:
    ...
    cols = [c for c in num_cols if c not in excluded]
    cols = [c for c in cols if "ttf" not in str(c).lower()]
    return cols
...
def compute_pr_curve(y_true: np.ndarray, y_prob: np.ndarray, max_points: int = 60) -> Optional[Dict[str, List[float]]]:
    try:
    ...
    if p.size == 0 or r.size == 0:
        return None
    idx = np.linspace(0, len(p) - 1, num=min(max_points, len(p))).astype(int)
    return {"precision": [float(p[i]) for i in idx], "recall": [float(r[i]) for i in idx]}
...
def train_streaming_sgd(
    ...
    while not (pos_found and neg_found) and scan_offset < max(train_count, total_rows):
        size = min(chunk, max(train_count, total_rows) - scan_offset)
...
def evaluate_streaming(
    ...
    if y_true.size == 0:
        return {
            ...
            "class_dist_eval": {"0": 0, "1": 0},
        }
    metrics = evaluate_predictions(y_true, y_pred, y_prob)
    c0 = int(np.sum(y_true == 0))
    c1 = int(np.sum(y_true == 1))
    metrics["class_dist_eval"] = {"0": c0, "1": c1}
...
def api_train():
    ...
    eval_offset_payload = payload.get("eval_offset")
    eval_count_payload = payload.get("eval_count")
    ...
    eval_offset = int(eval_offset_payload if eval_offset_payload is not None else int(splits.get("train_count") or 0))
    eval_count = int(eval_count_payload if eval_count_payload is not None else int(splits.get("eval_count") or 0))
    eval_offset = max(0, eval_offset)
    eval_count = max(1, eval_count)

    # allow auto-selection of features if none provided
    ...
    try:
        ...
        try:
            ...
            cur.execute(f"SELECT COUNT(1) FROM [{table_name}]")
            total_rows = int(cur.fetchone()[0])
            cur.execute(f"SELECT COUNT(1) FROM [{table_name}] WHERE COALESCE(CAST([{target}] AS REAL),0) > 0")
            pos_rows = int(cur.fetchone()[0])
            neg_rows = int(total_rows - pos_rows)
            if pos_rows == 0 or neg_rows == 0:
                return jsonify({"error": "Invalid target for classification: single class detected. Use 'fault_occurred'."}), 400
        except Exception:
            pass
        if not features:
            features = infer_feature_columns(df_head, target)
            if not features:
                return jsonify({"error": "No usable features found"}), 400
        else:
            for f in features:
        ...
        if fast_mode:
            ...
            if metrics.get("class_dist_eval", {}).get("1", 0) == 0:
                total_rows = 0
                try:
                    cur = conn.cursor()
                    cur.execute(f"SELECT COUNT(1) FROM [{table_name}]")
                    ...
                    total_rows = 0
                step = max(chunk_size, 50000)
                best = None
                offset = 0
                limit = int(eval_count)
                while offset + limit <= total_rows and offset < int(splits["train_count"]) + int(splits["eval_count"]):
                    m2 = evaluate_streaming(conn, table_name, model, features, target, medians, eval_count=limit, eval_offset=offset, chunk=chunk_size)
                    if m2.get("class_dist_eval", {}).get("1", 0) > 0:
                        best = m2
                        break
                    offset += step
                if best is not None:
            ...
            if oversample:
                bc = np.bincount(y_train, minlength=2)
                if bc[0] > 0 and bc[1] > 0 and bc[0] != bc[1]:
                    major = 0 if bc[0] > bc[1] else 1
                    minor = 1 - major
                    idx_minor = np.where(y_train == minor)[0]
                    add_n = int(abs(bc[major] - bc[minor]))
                    if add_n > 0 and idx_minor.size > 0:
                        add_idx = np.random.choice(idx_minor, size=add_n, replace=True)
                        X_train = np.vstack([X_train, X_train[add_idx]])
            ...
            c0 = int(np.sum(y_eval == 0))
            c1 = int(np.sum(y_eval == 1))
            metrics["class_dist_eval"] = {"0": c0, "1": c1}
            pr = compute_pr_curve(y_eval, y_prob)
            if pr is not None:
                metrics["pr_curve"] = pr
    ...
    try:
        blob = serialize_model_payload(model, list(features), dict(medians))
        with open(os.path.join(BASE_DIR, "fault_detection_model.pkl"), "wb") as f:
            f.write(blob)
    except Exception:
        pass
...
def api_train_default():
    ...
    conn = sqlite3.connect(db_path)
    try:
        df_head = load_sql_range(conn, table_name, 1000, 0)
        if target not in df_head.columns:
            return jsonify({"error": "Target column not found in table"}), 400
        features = infer_feature_columns(df_head, target)
        if not features:
        ...
        if fast_mode:
            ...
            from sklearn.ensemble import RandomForestClassifier
            train_df = load_sql_features(conn, table_name, features + [target], int(splits["train_count"]), 0)
            eval_df = load_sql_features(conn, table_name, features + [target], int(splits["eval_count"]), int(splits["train_count"]))
            train_df = transform_columns(train_df, features)
            eval_df = transform_columns(eval_df, features)
            medians = impute_median_fit(train_df, features)
            X_train = impute_median_apply(train_df, features, medians)
...
@app.get("/api/infer_summary")
def api_infer_summary():
    chunk_size = int(request.args.get("chunk_size") or 50000)
    chunk_size = max(1000, min(50000, chunk_size))
    with STATE_LOCK:
        if not STATE.connected:
            return jsonify({"error": "Not connected"}), 400
    ...
    try:
        ...
        while done < live_count:
            size = min(chunk_size, live_count - done)
            df = load_sql_features(conn, table_name, cols, size, offset)
            df = transform_columns(df, features)
            X = impute_median_apply(df, features, medians)
            try:
                prob = model.predict_proba(X)[:, 1]
            ...
            preds = model.predict(X)
            total += int(len(preds))
            faults += int(np.sum(preds))
            take = min(10 - len(samples), len(preds))
            for i in range(take):
                samples.append({"index": offset - live_offset + i, "predicted_fault": int(preds[i]), "probability": float(prob[i])})
            done += size
            offset += size
            if len(samples) >= 10:
                pass
        ...
        conn.close()
    rate = (faults / total) if total > 0 else 0.0
    return jsonify({"total_rows": total, "predicted_faults": faults, "fault_rate": round(rate, 6), "samples": samples})


@app.post("/api/load_model")
def api_load_model():
    payload = request.get_json(silent=True) or {}
    path = str(payload.get("path") or os.path.join(BASE_DIR, "fault_detection_model.pkl"))
    ...
    try:
        ...
        return jsonify({"error": f"Load error: {e}"}), 400
    model = data.get("model")
    features = data.get("features") or []
    medians = data.get("medians") or {}
    if model is None or not features or not isinstance(medians, dict):
        return jsonify({"error": "Invalid model payload"}), 400
    with STATE_LOCK:
        STATE.model = model
        STATE.features = list(features)
        STATE.medians = dict(medians)
    return jsonify({"loaded": True, "feature_count": len(features)})
